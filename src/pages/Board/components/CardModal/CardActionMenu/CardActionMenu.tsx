...
import { ICardActionMenu } from '../../../../../common/interfaces/ICardActionMenu';
import { useAppSelector, useAppDispatch } from '../../../../../app/hooks';
import { triggerBoardRefresh, fetchBoard } from '../../../boardSlice';
import { InputComponent } from '../../../../Misc/InputComponent/InputComponent';
import { createCard, refreshList, removeCard } from '../../../../../api/request';
import './cardActionMenu.scss';
import { IBoardsList } from '../../../../../common/interfaces/IBoardsList';
import { AppDispatch } from '../../../../../app/store';
import { ILists } from '../../../../../common/interfaces/ILists';
import { IUpdatedCards } from '../../../../../common/interfaces/IUpdatedCards';
...
export function CardActionMenu({
  ...
  useEffect(() => {
    ...
    const newPositionOptions = Array.from({ length }, (item, index) => <option key={index + 1}>{index + 1}</option>);
  ...
  return (
    <div className="card-action-menu">
      <div className="action-menu-title">
        <p>{actionName}</p>
        <button className="close-btn" onClick={() => setIsCardActionMenuOpen(false)}>
          X
        </button>
      </div>
      {performedAction === 'copy' && (
        <>
          <p className="input-title">Назва</p>
          <InputComponent value={title} handleChange={handleInputChange} className="card-action-menu-input" />
        </>
      )}
      <div className="action-subtitle">{actionSubtitle}</div>
